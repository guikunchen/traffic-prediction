...
class TrafficDataset(Dataset):
    def __init__(self, data_path, num_nodes, divide_days, time_interval, history_length, train_mode):
        ...
        self.data_norm_base, self.flow_data = self.pre_process_data(data=get_flow_data(data_path[1]), norm_dim=1)
    ...
    def __getitem__(self, index):  # (x, y), index = [0, L1 - 1]
        ...
        data_x, data_y = TrafficDataset.slice_data(self.flow_data, self.history_length, index, self.train_mode)

        data_x = TrafficDataset.to_tensor(data_x)  # [N, H, D]
        data_y = TrafficDataset.to_tensor(data_y).unsqueeze(1)  # [N, 1, D]

        return {"graph": TrafficDataset.to_tensor(self.graph), "flow_x": data_x, "flow_y": data_y}
    ...
    def pre_process_data(data, norm_dim):
        ...
        data_norm_base, norm_data = TrafficDataset.normalize_data(data, norm_dim)  # normalize data

        return data_norm_base, norm_data

    @staticmethod
    def normalize_data(data, norm_dim):
        """
        :param norm_dim:
        :param data: np.array, original traffic data without normalization.
        :return:
        ...
        return (max_data, min_data), normalized_data

    @staticmethod
    def inverse_normalize_data(max_data, min_data, data):
        """
...
if __name__ == '__main__':
    train_data = TrafficDataset(data_path=["PeMS_04/PeMS04.csv", "PeMS_04/PeMS04.npz"], num_nodes=307, divide_days=[45, 14],
